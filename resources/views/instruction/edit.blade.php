...
@section('title', 'List - Instruction')

@section('content_header')
<h1>Instruction Deatils</h1>
@endsection
...
<div class="row">
  ...
  <form action="{{ route('instruction.update', $instruction->id) }}" method="post">
    ...
    <div class="col-md-6">
      ...
      <div class="box box-primary">
        <div class="box-header with-border">
          <h3 class="box-title">Update Instruction Details</h3>
        </div>
        <!-- /.box-header -->
        <div class="box-body">
          <div class="form-group input-group date {{ $errors->has('case_datetime') ? 'has-error' : ''}}">
            <label for="case_datetime">Instruction Date Time</label>
            <input type="text" class="form-control datetimepicker {{ $errors->has('case_datetime') ? 'has-error' : ''}} " name="case_datetime" id="case_datetime" placeholder="Instruction Date Time" value="{{ old('case_datetime', $instruction->case_datetime->format('d/m/Y h:i A')) }}">
          ...
          {{-- <div class="form-group {{ $errors->has('consignee_id') ? 'has-error' : ''}}">
            ...
            <select class="form-control" id="consignee_id" name="consignee_id">
              ...
              @foreach ($consignees as $consignee)
               <option value="{{$consignee->id}}" {{ $consignee->id == old('consignee_id', $instruction->consignment->consignee_id) ? 'selected' : ''}}>{{$consignee->name }}</option>
              @endforeach
            </select>
          ...
          <div class="form-group {{ $errors->has('loading_port_id') ? 'has-error' : ''}}">
            ...
            <select class="form-control" id="loading_port_id" name="loading_port_id">
              ...
              @foreach ($ports as $port)
               <option value="{{$port->id}}" {{ $port->id == old('loading_port_id', $instruction->consignment->loading_port_id) ? 'selected' : ''}}>{{$port->name }}</option>
              @endforeach
            </select>
          ...
          <div class="form-group {{ $errors->has('discharge_port_id') ? 'has-error' : ''}}">
            ...
            <select class="form-control" id="discharge_port_id" name="discharge_port_id">
              <option value="">Select Port</option>
              @foreach ($ports as $port)
               <option value="{{$port->id}}" {{ $port->id == old('discharge_port_id', $instruction->consignment->discharge_port_id) ? 'selected' : ''}}>{{$port->name }}</option>
              @endforeach
            </select>
         ...
         <div class="form-group {{ $errors->has('shipping_company_id') ? 'has-error' : ''}}">
            ...
            <select class="form-control" id="shipping_company_id" name="shipping_company_id">
              ...
              @foreach ($shipping_company_list as $company_list)
               <option value="{{$company_list->id}}" {{ $company_list->id == old('shipping_company_id',$instruction->shipping_company_id) ? 'selected' : ''}}>{{$company_list->name }}</option>
              @endforeach
            </select>
           ...
           <div class="form-group {{ $errors->has('supplier_id') ? 'has-error' : ''}}">
            ...
            <select class="form-control" id="supplier_id" name="supplier_id">
              ...
              @foreach ($supplier_list as $supplier)
               <option value="{{$supplier->id}}" {{ $supplier->id == old('supplier_id', $instruction->supplier_id) ? 'selected' : ''}}>{{$supplier->name }}</option>
              @endforeach
            </select>
          ...
          {{-- <div class="form-group {{ $errors->has('weight') ? 'has-error' : ''}}">
            ...
            <input type="text" class="form-control" name="weight" id="weight" placeholder="Weight" value="{{ old('weight', $instruction->weight) }}">
            @if ($errors->has('weight'))
              <span class="help-block text-danger">{{ $errors->first('weight') }}</span>
            @endif
          </div> --}}
          <div class="form-group input-group date{{ $errors->has('pickup_datetime') ? 'has-error' : ''}}">
            <label for="pickup_datetime">Pickup Date Time</label>
            <input type="text" class="form-control datetimepicker {{ $errors->has('pickup_datetime') ? 'has-error' : ''}} " name="pickup_datetime" id="pickup_datetime" placeholder="Pickup Date Time" value="{{ old('pickup_datetime', $instruction->pickup_datetime->format('d/m/Y h:i A')) }}">
          ...
          <div class="form-group {{ $errors->has('door_orientation') ? 'has-error' : ''}}">
            ...
            <select class="form-control" id="door_orientation" name="door_orientation">
              ...
              <option {{ "Towards Rear" == old('door_orientation', $instruction->door_orientation) ? 'selected' : ''}}>Towards Rear</option>
              <option {{ "Towards Cab" == old('door_orientation', $instruction->door_orientation) ? 'selected' : ''}}>Towards Cab</option>
            </select>
            ...
            <div class="form-group {{ $errors->has('no_of_containers') ? 'has-error' : ''}}">
                ...
                <input type="text" class="form-control" name="no_of_containers" id="no_of_containers"
                       placeholder="No of Containers"
                       value="{{ old('no_of_containers', $instruction->no_of_containers) }}">
          ...
          <div class="form-group {{ $errors->has('instructions') ? 'has-error' : ''}}">
            <label for="instructions">Instructions</label>
              <textarea class="textarea" name="instructions" placeholder="Instructions"
                    style="width: 100%; height: 200px; font-size: 14px; line-height: 18px; border: 1px solid #dddddd; padding: 10px;" required="">{{ old('instructions', $instruction->instructions) }}</textarea>
    ...
    {{-- <div class="col-md-6">
      ...
      <div class="box box-primary">
        <div class="box-header with-border">
          <h3 class="box-title">Update Consignment Items</h3>
        </div>
        <!-- /.box-header -->
            ...
            @foreach($instruction->consignment->consignmentDetails as $item)
              <fieldset class="consignment_item">
                ...
                <div class="form-group input-group {{ $errors->has('description') ? 'has-error' : ''}}">
                  <label for="description">Item Description *</label>
                  <input type="text" class="form-control {{ $errors->has('description') ? 'has-error' : ''}} " name="description[]" placeholder="Enter Item Description" value="{{ $item->description }}">
                ...
                <div class="form-group input-group {{ $errors->has('item_weight') ? 'has-error' : ''}}">
                  <label for="item_weight">Total Qty. (Weight) *</label>
                  <input type="text" class="form-control item_weight calculate {{ $errors->has('item_weight') ? 'has-error' : ''}} " name="item_weight[]" placeholder="Enter Weight in KG" value="{{ $item->item_weight }}">
                ...
                <div class="form-group input-group {{ $errors->has('price') ? 'has-error' : ''}}">
                  ...
                  <input type="text" class="form-control price calculate {{ $errors->has('price') ? 'has-error' : ''}} " name="price[]" placeholder="Enter Price" value="{{ $item->price }}">
                  @if ($errors->has('price'))
                ...
                <div class="form-group input-group {{ $errors->has('total_price') ? 'has-error' : ''}}">
                  <label for="total_price">Total Price ($)</label>
                  <input type="text" class="form-control total_price {{ $errors->has('total_price') ? 'has-error' : ''}}" disabled="" name="total_price[]" placeholder="Total Price" value="{{ $item->item_weight * $item->price }}" disabled="">
