          ...
          <div class="box">
            ...
            <div class="box-body">
              <table id="shippingCompany" class="table table-bordered table-hover">
                ...
                <tbody>
                   @foreach ($port_detail as $port_details)
                  <tr>
                    <td>{{ $port_details->id }}</td>
                    <td>{{ $port_details->name }}</td>
                    <td>{{ $port_details->country->name ?? '-' }}</td>
                    <td>{{ $port_details->city->name ?? '-' }}</td>
                    {{-- <td>{{ $port_details->address ?? '-' }}</td> --}}
                    <td class="text-center">
                      <div class="btn-group action-buttons"> 
                         <a href="{{ route('port.show', $port_details->id) }}" >
                          <button class="btn btn-xs btn-success" type="button" data-toggle="tooltip" title="Show"><i class="fa fa-eye"></i>
                          </button>
                        </a>
                        <a href="{{ route('port.edit', $port_details->id) }}">
                          <button class="btn btn-xs btn-primary" type="button" data-toggle="tooltip" title="Edit"><i class="fa fa-pencil"></i>
                          </button>
                        </a> 
                        <form action="{{ route('port.destroy',$port_details->id) }}" method="post" class="form-inline remove">
                          @csrf
                          @method('delete')
