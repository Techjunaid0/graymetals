    ...
    <div class="row">
        <div class="col-md-12">
            ...
            <div class="box box-primary">
                ...
                <!-- /.box-header -->
                <!-- form start -->
                <form action="{{ route('shipping_co.update', $shipping_company->id) }}" method="post">
                    @csrf
                    @method('put')
                    <div class="box-body">
                        <div class="form-group {{ $errors->has('name') ? 'has-error' : ''}}">
                            ...
                            <input type="text" class="form-control {{ $errors->has('name') ? 'has-error' : ''}} "
                                   ...
                                   value="{{ old('name', $shipping_company->name) }}">
                            @if ($errors->has('name'))
                        ...
                        <div class="form-group {{ $errors->has('phone') ? 'has-error' : ''}}">
                            ...
                            <input type="text" class="form-control" name="phone" id="phone" placeholder="Company Phone"
                                   value="{{ old('phone', $shipping_company->phone) }}">
                            @if ($errors->has('phone'))
                                <span class="help-block text-danger">{{ $errors->first('phone') }}</span>
                            @endif
                        </div>
                        <div class="form-group {{ $errors->has('primary_email') ? 'has-error' : ''}}">
                            <label for="primary_email">Company Primary Email *</label>
                            <input type="text" class="form-control" name="primary_email" id="primary_email"
                                   placeholder="Company Primary Email"
                                   value="{{ old('primary_email', $shipping_company->primary_email) }}">
                        ...
                        <div class="form-group {{ $errors->has('postal_code') ? 'has-error' : ''}}">
                            ...
                            <input type="text" class="form-control" name="postal_code" id="postal_code"
                                   ...
                                   value="{{ old('postal_code', $shipping_company->postal_code) }}">
                        ...
                        <div class="form-group {{ $errors->has('country_id') ? 'has-error' : ''}}">
                            ...
                            <select class="form-control" id="country_id" name="country_id">
                                ...
                                @foreach ($countries as $country)
                                    <option
                                        value="{{$country->id}}" {{ $country->id == old('country_id', $shipping_company->country_id) ? 'selected' : ''}}>{{$country->name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('address') ? 'has-error' : ''}}">
                            ...
                            <input type="text" class="form-control" name="address" id="address"
                                   ...
                                   value="{{ old('address', $shipping_company->address) }}">
...
@push('js')
    <script type="text/javascript">
        $(function () {
            ...
            setTimeout(function () {
                var old_state_id = '{{ old('state_id', $shipping_company->state_id) }}';
                var old_city_id = '{{ old('city_id', $shipping_company->city_id) }}';
                if (old_state_id != '') {
